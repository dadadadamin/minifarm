...
class ConversationService(
    ...
    private fun startThinking(text: String) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            // 입력 텍스트 전처리 과정 - 문장에서 마지막에 있는 "."과 "?"을 제거
            ...
            //결과 파싱
            var llmResponseList = LLMResponseParser.llmResponseParse(response)
            var ttsText = ""        //최종 발화할 텍스트

            if (llmResponseList.isNotEmpty()) {
                val item = llmResponseList[0]
                val token = item.token
                val type = item.parameters["type"] // 1:ON, 2:OFF
                val obj = item.parameters["object"] // 1:환기팬, 2:펌프, 3:LED ...

                // ----------------------------------------------------
                // [Case 1] 기기 제어 -> WebSocket 사용 (<maum_0>, <maum_1>, <maum_2>)
                // ----------------------------------------------------
                if (token == "<maum_0>" || token == "<maum_1>" || token == "<maum_2>") {
                    val isOn = (type == "1")

                    // 데이터시트 ID 매핑
                    val deviceName = when(token) {
                        "<maum_0>" -> "pump" // 펌프
                        "<maum_1>" -> "led"  // LED
                        "<maum_2>" -> "fan"  // 팬
                        else -> "unknown"
                    ...
                    if (deviceName != "unknown") {
                        // com.suda.agent 패키지에 있는 DeviceController 호출
                        com.suda.agent.DeviceController.controlDevice(deviceName, isOn)
                    }
                ...
                // ----------------------------------------------------
                // [Case 2] 센서 조회 -> WebSocket 사용 (<maum_3>)
                // ----------------------------------------------------
                else if (token == "<maum_3>") {
                    // obj: 1(EC), 2(온도), 3(습도)...
                    val targetObj = obj ?: "0"

                    // 1. WebSocket으로 값 가져오기 (비동기 대기)
                    val value = com.suda.agent.DeviceController.getSensorValue(targetObj)

                    // 2. 통신 성공/실패 여부에 따라 말 바꾸기 (방어 코드)
                    if (value == "응답 없음" || value == "연결 안됨" || value == "연결 중...") {
                        // 실패 시: 사용자에게 에러 상황 알림
                        ttsText = "센서와 통신할 수 없습니다. 연결 상태를 확인해 주세요."
                        // (선택사항) 로그 남기기
                    }
                    else {
                        // 성공 시: 정상적으로 문장 조립
                        ttsText = when (targetObj) {
                            "1" -> "현재 EC 농도는 ${value}입니다."
                            "2" -> "현재 온도는 ${value}도입니다."
                            "3" -> "현재 습도는 ${value}퍼센트입니다."
                            "4" -> "현재 조도는 ${value}룩스입니다."
                            "5" -> "현재 이산화탄소 농도는 ${value}피피엠입니다."
                            else -> "해당 센서 정보를 찾을 수 없습니다."
                        }
                    }
                }

                // [전체 종료] <maum_6>
                else if (token == "<maum_6>"){
                    if (type == "1") {
                        // 1. 연결된 하드웨어들 먼저 끄기
                        com.suda.agent.DeviceController.controlDevice("fan", false)
                        com.suda.agent.DeviceController.controlDevice("pump", false)
                        com.suda.agent.DeviceController.controlDevice("led", false)

                        // 2. TTS로 종료 인사 먼저 하기
                        // (TTS가 끝날 때까지 약간의 딜레이가 필요할 수 있음 - 여기선 생략)
                        ttsText = "모든 장치를 끄고 시스템을 종료합니다."

                        // 3. 수다키트 본체 종료 (약간의 딜레이 후 실행 권장)
                        CoroutineScope(Dispatchers.IO).launch {
                            Thread.sleep(3000) // 3초 뒤에 꺼짐 (TTS 말할 시간 벌기)
                            com.suda.agent.DeviceController.shutdownSudaKit()
                        }
                    }
                }
                // 기타 (<maum_4> 등)
            ...
            // 방어 코드
            if (ttsText.isEmpty()) ttsText = response.replace("<maum_end>", "")
