...
-- 식물 별 생장 환경 저장 테이블
CREATE TABLE IF NOT EXISTS plants_info (
                                           id                  BIGSERIAL PRIMARY KEY,
...
--일단 이 부분은 필요없어 보임
--CREATE UNIQUE INDEX IF NOT EXISTS uq_user_plants_device
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_diary_user_plant_date
...
CREATE INDEX idx_sensor_logs_user_plant_created_at
    ON sensor_logs(user_plant_id, created_at DESC);


-- 6. diagnosis (AI 진단 결과)
-- source_type은 항상 MOBILE로 사용가능하지만 남겨둠
-- ------------------------------------------------------------
